...
static pixman_region32_t create_corner_location_region(int radius, enum corner_location corners, int x, int y, int width, int height) {
	pixman_region32_t corner_region;
	pixman_region32_init(&corner_region);
	if (corners & CORNER_LOCATION_TOP_LEFT) {
		pixman_region32_union_rect(&corner_region, &corner_region, x, y, radius, radius);
	}

	if (corners & CORNER_LOCATION_TOP_RIGHT) {
		pixman_region32_union_rect(&corner_region, &corner_region, x + (width - radius), y, radius, radius);
	}

	if (corners & CORNER_LOCATION_BOTTOM_LEFT) {
		pixman_region32_union_rect(&corner_region, &corner_region, x, y + (height - radius), radius, radius);
	}

	if (corners & CORNER_LOCATION_BOTTOM_RIGHT) {
		pixman_region32_union_rect(&corner_region, &corner_region, x + (width - radius), y + (height - radius), radius, radius);
	}
...
static void scene_node_opaque_region(struct wlr_scene_node *node, int x, int y,
	...
	if (node->type == WLR_SCENE_NODE_RECT) {
		...
		if (scene_rect->corner_radius > 0 && scene_rect->corners != CORNER_LOCATION_NONE) {
			pixman_region32_t corners = create_corner_location_region(scene_rect->corner_radius, scene_rect->corners,
				x, y, width, height);
		...
		if (scene_buffer->corner_radius > 0 && scene_buffer->corners != CORNER_LOCATION_NONE) {
			pixman_region32_t corners = create_corner_location_region(
